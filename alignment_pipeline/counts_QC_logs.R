...
names(counts_merged) <- c('Barcode.ID', rev(filters))
n.barcodes <- dim(counts_merged)[1]

plot_sample <- rep(counts_merged$Barcode.ID, 3)
...
if ( strtrim(platePrefix, 2) == "DP" ) {
  ...
  deepCovariates$RawReads     <- out_data$raw
...
write.table(out_data[-grep('raw', colnames(out_data))], file=paste(platePrefix, '_QC_counts.txt', sep=''), quote=FALSE, row.names=FALSE, sep='\t')
